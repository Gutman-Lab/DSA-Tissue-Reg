...
# bdsa_url: http://bdsa.pathology.emory.edu:8080/api/v1/item
# url2: tiles/thumbnail?width=1024&encoding=JPEG
# ftype: "accept: image/jpeg"
# folder_id: 673145a3900c0c05599c04e7
# size: 1024
# region: Hippocampus
# acc: ''
# passw: ''
...
# _ = gc.authenticate(acc, passw, interactive=False)

# token_info = gc.get("token/current")
# token_info = f"Girder-Token: {token_info['_id']}"
...
def register_fixed_moving(fixed_id, moving_id) -> np.array:
    ...
    fixed_data = return_byte(fixed_id)
    moving_data = return_byte(moving_id)

    fixed_img_sk = skimage.io.imread(io.BytesIO(fixed_data))
    moving_img_sk = skimage.io.imread(io.BytesIO(moving_data))
    ...
    axes[0].imshow(padded_mask1, cmap='gray')
    axes[0].set_title('Padded Binary Mask 1')
    axes[0].axis('off')
    axes[1].imshow(padded_mask2, cmap='gray')
    axes[1].set_title('Padded Binary Mask 2')
    axes[1].axis('off')
    plt.tight_layout()
    plt.show()"""
    ...
    # item_np_schema = item_meta['npSchema']
    # item_np_schema['fixedId'] = fixed_id
    # item_np_schema['size'] = 1024
    # item_np_schema['rotation'] = rotation_degrees
    # item_np_schema['offsetX'] = offset_x
    # item_np_schema['offsetY'] = offset_y
    # item_np_schema['scalingFactor'] = scaling
    # item_meta['npSchema'] = item_np_schema
    ...
    axes[1].axis('off')
    fig.suptitle(f'Init Dice: {round(prev_dice, 2)}, registered Dice: {round(dice,2)}', fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    # fig.savefig(f'registered/{case}/{subcase}/{reg_name}_dice.jpg')
    plt.show()"""
    ...
    plt.figure()
    plt.imshow(np_image, cmap="gray")
    plt.imshow(reg_image, cmap="jet", alpha=0.5)
    plt.title("Registered Overlay")
    plt.show()
